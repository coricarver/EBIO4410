...
# General information
This lab is due Wednesday, January 27th by 11:59 pm and is worth 5 points. You must upload your .rmd file and your knitted PDF to the assignment folder on Canvas.
...
For reproducibility, it is best to type all non-exploratory code in an R Script or R Markdown file. That way, you can go back and follow each step. For labs, all final code must be written in the R Markdown document. Otherwise, it will be impossible to know what you did. To do so, we can type code directly into the R Markdown document. To separate code from text, we use the following syntax: ` ```{r} ` with curly brackets around the r begins a part of code and ` ``` ` denotes the end of code. A section of code delimited in this way in a .Rmd is called a code chunk, or more simply a "chunk". Chunks are visually demarcated from text within a .Rmd file by appearing in a different color background. When you export (or "knit") the .rmd to a PDF, the code is evaluated and output is displayed. 
...
Note also that the ` ```{r}` and ````` do not render (print) in the pdf, but rather tell R that this is a code section. 

I know this is a bit confusing, but the pound sign means something different depending on whether it is inside our outside a chunk of R code in a .Rmd file.  Inside a code chunk, the pound symbol indicates that you are writing comments that should not ignored as a command. This helps you (or others) follow your logic. Outside of a code chunk, a # preceding a line of text inserts a section break within the .Rmd and will output the line of text as a header (appears as bigger, bold text) for that section in the knitted document. You will see this above with the General Information header, etc. By changing the number of pound signs that precede text you can change the type of header that is displayed in the knitted document (first level header vs. subsequent level subheaders). See how the *Basics of R* header and *Inserting a Chunk of Code* sub-header are coded above and how they appear in the knitted pdf file.  

## Running a Chunk of Code
In a .Rmd file you have text outside of chunks that is just evaluated as text and code within chunks that is designed to run (i.e. do something).  So how do you get R to run (i.e. evaluate) the code in one of your chunks.  In RStudio, the Console is the business end that does all of the running of code.  It is in the window below.  If you want to run some code from your .Rmd file you can copy and paste it in the Console and hit enter.  This will run this code.  Alternatively, you can type directly into the Console.  In RStudio, however, there are some shortcuts that make this easier.  For example, you can place the cursor in a line of code or highlight multiple lines of code and then select 'Run' and 'Run Selected Lines' to run those lines of code.  Alternatively, you can place your cursor in the line of code you want to evaluate or highlight multiple lines of code and then press Ctrl+Enter (Windows) or Command+Return (Mac). This will run this line of code.  You can see what was done in the Console window below.  You do not need to "knit" your R markdown into PDF each time you evaluate code. 
...
Additionally, if you type ? followed by an R command in the console, R will open documentation on the command. [Note: "commands" are functions in R that perform some type of task, such as *mean()*, *sum()*, or *plot()*]. 
For example:
...
Since R is open-source, there are great resources and question forums online as well. If there is a function you think R should be able to do, you are probably right! Google is a great resource for R. Also, stack overflow (stackoverflow.com) has answers to thousands of questions from R users. Likely someone has asked a similar question that is already answered on stack overflow.
...
Note that this finds the last element in this vector because $length(ex4)$ is equal to $14$ and $ex4[14]$ just returns the 14th element in the vector.  All we have done is to place one command within another.  This is a helpful feature of coding that we will use a lot.
...
\textcolor{red}{Q4: Now, add a fourth bird to our vector of birds. If you don't know, try googling first! Hint: what element number should it be? Print the vector to make sure it worked.}

```{r}
birds[4] = "Black-capped Chickadee"
birds[4]
```
...
Let's go ahead and look at one of R's example datasets, called CO2. You can print the whole dataframe if you type CO2 in your console (make sure you pay attention to capitalization!).
...
*The first column is plant, then type, then treatment, then CO2 concentration and finally C02 uptake.*
...
Some other functions you will often use with dataframes:
  ...
  - Summarize a dataset or vector using `summary()`.
  
  - Perform calculations on certain columns, such as `mean()`, `min()`, `max()`, `sd()`. 
  
  - Count the number of records in a column using `length()`.
...
```{r}
chilled.highConc <- subset(CO2ex, Treatment =="chilled" & conc > 500)
chilled.highConc
nrow(chilled.highConc)
```

*There were 12 records chilled in ambient conditions greater than 500*
...
Often you will want to load your own data into R. R can read .csv and .txt files using base R commands but not .xls files (there are bundled functions called "R packages" written by other R users we can install and use to do things like read in .xls or .xlsx files directly, but for now we are focusing on commands that come with R, i.e. "base R" code). We will read in the file Pelts.csv, which can be found on the course Canvas page. This file contains a list of the number of recorded pelts collected by the Hudson Bay Company from $1752$ to $1819$.
...
*Pelts is a data frame with 68 observations of two variables (date and number of pelts).*
...
```{r}
#Create a subset of the pelts dataframe extracting the number of pelts harvested between 1752 and 1761 inclusively. 
numPelts <- subset(pelts, date > 1751 & date < 1762)

#Find the mean number of pelts harvested
avgNumPelts <- mean(numPelts$no.pelts)

#Print the mean number of pelts harvested
avgNumPelts
```
