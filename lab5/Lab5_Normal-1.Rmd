...
# Text wrap chunk comments when knit so don't run off of PDF page

# Notes: 
## > Install 'knitr' package (install.packages("knitr", dependencies = T)) if knitting breaks at this chunk 
## > You don't need to execute this code in RStudio manually when working through the assignment, it only applies to the knitted document and will execute when you knit
## > You can vary the width cutoff number to see how that changes output knitted

library(knitr)
opts_chunk$set(tidy = T, tidy.opts = list(width.cutoff = 60))

# You can also change default settings in markdown in the opts_chunk$set above. For example, if you wanted to change default figure dimensions or figure alignment and have it apply to all figures in the .Rmd, you could do that inside the list() in the code above.
```
