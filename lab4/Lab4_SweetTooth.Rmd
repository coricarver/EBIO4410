...
*Null hypothesis: Students cannot identify Jelly Belly flavors better than chance (1/6th).*

*Alternative hypothesis: Students can identify Jelly Belly flavors better than chance.*
...
```{r}
jellyBeanData <- read.csv("JellyBelly.csv")
structure(jellyBeanData)

#Statistical Test Chosen: chi-squared goodness of fit test because it will compare a distribution of
#observed counts to a distribution of expected counts under some probability model.
guessedRight <- sum(jellyBeanData$Correct == 1)
guessedWrong <- sum(jellyBeanData$Correct == 0)
jellyBeanObserved <- c(guessedRight, guessedWrong)
# 1/6 is the probability that by random chance students could have guessed correctly
#5/6 is the probability that by random chance students could have guessed incorrectly
nullProp <- c(1/6, 5/6)
sum(nullProp) #sanity check

#Let's plot
jellyBeanExpected <- nullProp * sum(jellyBeanObserved)
jellyBeanExpected
# combine data; row 1 is observed, row 2 is expected
beanData <- rbind(jellyBeanObserved, jellyBeanExpected) 
# give the table column names
colnames(beanData)<- c("Correct", "Incorrect")
beanData

colors<-c("slateblue2", "orchid")
barplot(height = beanData, beside=TRUE, cex.names = 0.8, ylim = c(0,50),
        legend = c("Observed", "Expected"), col=colors)

chisq.test(x = jellyBeanObserved, p = nullProp)

```

*P-value: 2.992e-07, Chi-squared: 26.255* 
...
*The output of this tells us that there is a very small chance (less than 0.01%) of observing our jelly bean distribution OR jelly bean distributions even further from the null, if the null hypothesis is true. Since it is < 0.05, we reject the null hypothesis. Students CAN identify Jelly Belly flavors better than chance. Our test statistic is 26.3. *
...
*Null hypothesis: Taste ability does not differ between frequent, occasional and non-soda-drinkers.* 

*Alternative hypothesis: Taste ability does differ between frequent, occasional and non-soda-drinkers.*
...
```{r}
sodaData <- read.csv("CokevPepsi.csv")
head(sodaData)
...
mosaicplot( sodaTable, col = c("firebrick", "pink"), cex.axis = 1, 
            sub = "Soda Intake Frequency", ylab = "Ability to Correctly Taste", 
            main = "Soda Intake Frequency and Taste Ability")

##Chi Test
SodaChiTest <- chisq.test(y = sodaData$Correct, x = sodaData$SodaFrequency, correct = FALSE)
SodaChiTest
```

*P-value: 0.4083, Test Statistic: 1.7917* 
...
*The output of this tells us that there is a 41% chance of observing as big of a difference in taste ability (or even bigger differences) between soda intake frequency categories if there really were no link between soda intake frequency and taste ability. Since it is > 0.05, we cannot reject the null hypothesis. Taste ability does not differ between frequent, occasional, and non-soda-drinkers. Our test statistic is 1.7917.*
